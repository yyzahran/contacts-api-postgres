...
const fetchContact = async (req, res) => {
    const { params: { id: contactId }, user: { userId, name } } = req

    const contact = await getContactById(contactId, userId)
    if (!contact) {
        throw new NotFoundError(`No contact with id ${contactId} found for user ${name}.`)
    }
...
const fetchAllContacts = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json({ contactsList, count: contactsList.length })
}
